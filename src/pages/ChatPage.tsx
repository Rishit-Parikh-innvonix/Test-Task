...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      // 👇 NEW: skip loading if WebSocket just set it
      if (suppressNextMessageLoad) {
        setSuppressNextMessageLoad(false); // reset flag
        return;
      }
    ...
    wsStore.onMessage((msg: any) => {
      if (msg.type === 'chunk') {
        ...
        if (msg.conversation_id && !liveConversationId) {
          ...
          setSuppressNextMessageLoad(true); // ✅ skip fetch
